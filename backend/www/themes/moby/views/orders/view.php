...
<div class="col-lg-4">
    ...
    //                'name'=>Products::model()->getAttributeLabel('title'),
                    ...
                    'footer'=>CHtml::dropDownList('product_id','',Products::getTree(),array(
                        'data-order-id' => Yii::app()->request->getParam('id'),
                        'style'=>'width:120px',
                        'empty'=>''
                    ))
                ),
    ...
    //                'name'=>CardProduct::model()->getAttributeLabel('product_count'),
                    'value'=>function($data){
                        echo CHtml::beginForm(array('orders/addProduct'), 'POST', array('class' => 'calculate-form'));
                        echo CHtml::hiddenField('order_id', $data->order_id);
                        echo CHtml::hiddenField('product_id', $data->product->id);
                        echo CHtml::hiddenField('count', -1);
                        echo CHtml::link('', '#', array(
                            'class' => 'option glyphicon glyphicon-minus',
                            'submit' => array('orders/addProduct', 'id' => $data->order_id),
                            'confirm' => 'Вы уверены?'
                        ));
                        echo CHtml::endForm();
                        ...
                        echo CHtml::hiddenField('count', 1);
                        echo CHtml::link('', '#', array(
                            'class' => 'option glyphicon glyphicon-plus',
                            'submit' => array('orders/addProduct', 'id' => $data->order_id),
                            'confirm' => 'Вы уверены?'
                        ));
                        echo CHtml::endForm();
                ...
                array(
                    ...
                    'value'=>'$data->price',
                    'footer' => CHtml::link('+', $this->createUrl('addProduct') ,array('id'=>'add-order-product','class' => 'btn btn-primary btn-xs', 'name' => 'create'))
                ),
            ),
        )); ?>
    </fieldset>
    <fieldset>
        ...
        <?php $this->widget('zii.widgets.grid.CGridView', array(
            ...
            'dataProvider' => new CArrayDataProvider($model->gifts, array(
                'keyField' => 'gift_id',
            )),
            'summaryText' => false,
            'htmlOptions'=>array(
