...
<table class="table table-striped">
    <?php foreach($data['statuses'] as $status):?>
        <tr>
            <td class="col-lg-2"><?php echo SHtml::toHumanDate($status['date']) ?></td>
            <td class="col-lg-4"><?php echo $status['place']; echo $status['zipCode'] ? ', '.$status['zipCode'] : '' ?></td>
            <td class="col-lg-6"><?php echo $status['operation']['name']; echo isset($status['attribute']) ? ' / '.$status['attribute']['name'] : ''?></td>
        </tr>
    <?php endforeach ?>
</table>