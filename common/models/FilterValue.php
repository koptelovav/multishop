 ...
 *
 * The followings are the available columns in table 'filter_value':
 * @property string $id
...
class FilterValue extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id, filter_id, value', 'required'),
			array('id, filter_id', 'length', 'max'=>10),
			array('value', 'length', 'max'=>128),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, filter_id, value', 'safe', 'on'=>'search'),
		);
	}
