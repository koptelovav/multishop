...
class GDToolkit extends Toolkit {
    ...
    public function normalize($source, $destination, $width, $height){
        ...
        $x = ($width-$sourceInfo['width']) / 2;
        $y = ($height-$sourceInfo['height']) / 2;

        imageCopy($target,$sourceImage, $x, $y, 0, 0, $sourceInfo['width'], $sourceInfo['height']);
     ...
     * Water marks a file
     */
    function watermark($source, $destination, $pngWatermark, $x=0, $y=0, $repeat=false) {
        $sourceInfo = $this->getInfo($source);
        $sourceImage = $this->open($source, $sourceInfo['extension']);
        ...
        if($repeat){
            $img_paste_x = $x;
            while($img_paste_x < $sourceInfo['width']){
                $img_paste_y = $y;
                while($img_paste_y < $sourceInfo['height']){
                    imageCopy($target, $waterImage, $img_paste_x, $img_paste_y, 0, 0, $waterInfo['width'], $waterInfo['height']);
                    $img_paste_y += $waterInfo['height'];
                }
                $img_paste_x += $waterInfo['width'];
            }
        }else{
