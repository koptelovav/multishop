<?php
class BambooImage extends CApplicationComponent {
    ...
    public function createPath($presetName, $file) {
        ...
        $width = Yii::app()->shop->images->{$presetName.'_width'};
        $height = Yii::app()->shop->images->{$presetName.'_height'};
        if ($width && $height) {
            $basename = basename($file);
            $targetPath = Yii::getPathOfAlias($this->cacheIn.'.'.$presetName);
            ...
            if (file_exists($targetFile)) {
                ...
                if(is_file($file) && Yii::app()->shop->images->watermark) {
                    $x = 0;
                    $y = 0;

                    $this->waterMark($targetFile, $targetFile,
                        str_replace('\\', '/', Yii::app()->media->basePath .'/www'. Yii::app()->shop->images->watermark),
                        $x,
                        $y);
                }
                $this->scaleAndNormalize($targetFile, $targetFile, $width, $height);
                return $targetFile;
            }
        } else {
            return false;
    ...
    public function createUrl($presetName, $file) {
        ...
        $webrootPath = Yii::getPathOfAlias('media.www');
        if(strpos($generatedPath, $webrootPath) !== FALSE) {
            $generatedPath = substr($generatedPath, strlen($webrootPath));
        }
        $generatedUrl = str_replace('\\', '/', Yii::app()->media->baseUrl.$generatedPath);
    ...
    public function scaleAndNormalize($source, $destination, $width, $height) {
        $result = $this->scale($source, $destination, $width, $height);
        if($result)
            if($height == 1200)
                return $result;
            else
                return $this->toolkit->normalize($source, $destination,$width, $height);
        else
            return false;
    }
    ...
    public function watermark($source, $destination, $pngWatermark, $x=0, $y=0) {
       return $this->toolkit->watermark($source, $destination, $pngWatermark, $x, $y, true);
    }
