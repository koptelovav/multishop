...
class RGenerator extends CApplicationComponent
{
	...
	*/
	protected function getAllControllers()
	{
        ...
        if(Rights::getAppId())
            $items['controllers'] = $this->getControllersInPath($basePath.DIRECTORY_SEPARATOR.Rights::getAppId().DIRECTORY_SEPARATOR.'controllers');
        else
				...
				if( $entry{0}!=='.' )
				{
					...
					if( strpos(strtolower($entry), 'controller')!==false )
					{
						...
						$controllers[ strtolower($name) ] = array(
							'name'=>Rights::getAppId().'.'.$name,
							'file'=>Rights::getAppId().'.'.$entry,
							'path'=>$entryPath,
						);
					}
